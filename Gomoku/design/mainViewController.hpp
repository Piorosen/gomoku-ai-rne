...
class mainViewController : public grc::viewcontroller
{
private:
    std::shared_ptr<grc::view> makeMainMenuView()
    {
        ...
        continueButton->buttonDown = [this]()
        {
            if (this->buttonContinueGame)
            {
                this->buttonContinueGame();
            }
        };
        grc::rect f4;
        ...
        aimatchingButton->buttonDown = [this]()
        {
            if (this->buttonAiMatch)
            {
                this->buttonAiMatch();
            }
        };
    ...
    void setDesign()
    {
        ...
        view.push_back(std::make_shared<grc::view>(f4, grc::color(0xffffffff)));
        view.push_back(std::static_pointer_cast<grc::view>(logo));
        view.push_back(std::static_pointer_cast<grc::view>(start));
        view.push_back(std::static_pointer_cast<grc::view>(tree));
    }

public:
    std::function<void()> buttonTree;
    std::function<void()> buttonNewGame;
    std::function<void()> buttonContinueGame;
    std::function<void()> buttonAiMatch;
